 ...
 *
 * -- Allocation --
 * Allocate a block that is actually 4 bytes larger than requested. Store the
 * size of the allocated block in the first 4 bytes of the block; call this
 * number N. The smallest granularity of allocation supported is 4 bytes, so the
 * low two bits of the 4-byte size are use for other purposes. The lowest order
 * bit is a reference bit. If it is set, it means the next N & ~0x3 bytes of
 * memory afterwards are in use. If it is not set, then the next N & ~0x3 bytes
 * of memory are free to be allocated by malloc.
 */
