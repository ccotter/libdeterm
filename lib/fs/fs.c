        ...
        blockno_t blkno, blockno_t *slot, void *dst, int flush, int clear)
{
    ...
    if (0 != *slot && flush)
    {
        if (0 > dput(childid, DETERMINE_VM_COPY, dst, FS_BLOCK_SIZE, blockaddr(*slot)))
            return -E_COPY_ERROR;
    }
    if (0 == blkno)
        return 0;
    if (0 > dget(childid, DETERMINE_VM_COPY, blockaddr(blkno), FS_BLOCK_SIZE, dst))
        return -E_COPY_ERROR;
    if (clear)
...
static int get_child_block(pid_t childid, struct inode *inode, uint32_t blockno, void *dst)
{
    ...
    if ((rc = fetch_child_block(childid, inode, inode->f_double_indirect, &fs_state->s_double_indirect_slot, diblk, 1, 0)))
        return rc;
...
static int dir_unlink(struct inode *dir, const char *name, ssize_t n)
{
    ...
    while (dentry && block < nblocks)
    {
        ...
        for (j = 0; j < NDIRENTRIES_PER_BLOCK; ++j)
        {
            ...
            if ((n < 0 && 0 == strcmp(name, dentry[j].d_name)) || (n >= 0 && 0 == strncmp(name, dentry[j].d_name, n)))
            {
                to_remove = &dentry[j];
            }
            last = &dentry[j];
...
int dfs_init_clean(void)
{
    ...
    /*iprintf("FS SIZE %08lx\n", FS_SIZE);
    iprintf("Mapped %08lx %08lx\n",FS_VM_START, FS_VM_START + FS_SIZE);
    iprintf("Mapped %08lx %08lx\n",FS_SCRATCH1, FS_SCRATCH1 + FS_SCRATCH1_SIZE);
    iprintf("Mapped %08lx %08lx\n",FS_SCRATCH2, FS_SCRATCH2 + FS_SCRATCH2_SIZE);
    iprintf("Mapped %08lx %08lx\n",FS_SCRATCH3, FS_SCRATCH3 + FS_SCRATCH3_SIZE);
    iprintf("Mapped %08lx %08lx\n",FS_SCRATCH4, FS_SCRATCH4 + FS_SCRATCH4_SIZE);
    iprintf("Mapped %08lx %08lx\n",FS_SCRATCH5, FS_SCRATCH5 + FS_SCRATCH5_SIZE);*/
...
static int _dfs_write(struct file_state_struct *state, const void *buf, size_t len)
{
    struct inode *f;
...
int dfs_put(pid_t childid)
{
    ...
    if (0 > (rc = dput(childid, DETERMINE_VM_COPY, copy_start, copy_size, copy_start)))
        return -E_COPY_ERROR;
    ...
    if (0 > (rc = dput(childid, DETERMINE_VM_COPY, copy_start, copy_size, copy_start)))
        return -E_COPY_ERROR;
    /* Copy the file state. */
    if (0 > (rc = dput(childid, DETERMINE_VM_COPY, fs_state, sizeof(struct dfs_state_struct), fs_state)))
        return -E_COPY_ERROR;
    ...
    if (0 > (rc = dput(childid, DETERMINE_VM_COPY, cinodes, FS_NINODES * FS_INODE_SIZE, copy_start)))
        return -E_COPY_ERROR;
    /* Map the scratch spaces so that the virtual memory space is valid in the child. */
    if (0 > (rc = dput(childid, DETERMINE_VM_COPY, (void*)FS_SCRATCH1, FS_STATE - FS_SCRATCH1, (void*)FS_SCRATCH1)))
        return -E_COPY_ERROR;
...
static int reconcile_append(struct inode *pinode, struct inode *cinode, pid_t childid)
{
    ...
    //iprintf("TO APP %d %d %d %d\n", pinode->f_size, cinode->f_size, pinode->f_size -
    //    pinode->f_append_off, cinode->f_size - cinode->f_append_off);
    ppos = pinode->f_size;
    ...
    if ((rc = fetch_child_block(childid, cinode, 0, &fs_state->s_indirect_slot, (void*)SCRATCH_INDIRECT, 1, 0)))
        goto end;
    if ((rc = fetch_child_block(childid, cinode, 0, &fs_state->s_double_indirect_slot, (void*)SCRATCH_DOUBLE_INDIRECT, 1, 0)))
        goto end;
    ...
    //iprintf("APPENDED %d %d\n",cinode->f_size, pinode->f_size);
    update_file_version(cinode);
    update_file_version(pinode);
    //iprintf("AFTER{%d,%d %d,%d}", );
...
int dfs_get(pid_t childid)
{
    ...
    if (0 > dget(childid, DETERMINE_VM_COPY, pinodes, FS_NINODES * FS_INODE_SIZE, cinodes))
        return -E_COPY_ERROR;
    /* Copy bitmap from child process. */
    if (0 > dget(childid, DETERMINE_VM_COPY, blockaddr(2), FS_BITMAP_BLOCKS * FS_BLOCK_SIZE, cbitmap))
        return -E_COPY_ERROR;
    ...
    for (ino = 0; ino < FS_NINODES; ++ino)
    {
        ...
        //iprintf("Changed inode %d (%d, %d, %d)\n", ino, pinodes[ino].f_version, cinodes[ino].f_rversion, cinodes[ino].f_version);
        return -E_CONFLICT;
    }
    for (ino = 0; ino < FS_NINODES; ++ino)
    {
    ...
    if (0 > dput(childid, DETERMINE_VM_COPY, cinodes, FS_NINODES * FS_INODE_SIZE, pinodes))
        return -E_COPY_ERROR;
...
{
    //iprintf("proc state now %d %d\n", state, sizeof(struct dfs_state_struct));
    //iprintf("1234567891234567\n");
    int i;
...
int dfs_childstate(pid_t childid)
{
    ...
    int rc = dget(childid, DETERMINE_VM_COPY, (void*)FS_STATE, sizeof(struct dfs_state_struct), cstate);
    //iprintf("...mon=%d-%d\n",rc,cstate->proc_state);
    if (DETERMINE_S_READY != rc)
        return -E_COPY_ERROR;
    //iprintf("got state now %d\n", cstate->proc_state);
