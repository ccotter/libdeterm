...
#if 0
	void *a1 = larg.lo;
	ssize_t sz1 = (larg.hi - larg.lo + 1)  * sizeof(KEY_T);
	void *a2 = rarg.lo;
	ssize_t sz2 = (rarg.hi - rarg.lo + 1)  * sizeof(KEY_T);
	if (sz1 > 0)
		dget(lcn, DET_MERGE, (unsigned long)a1, sz1, 0);
	if (sz2 > 0)
		dget(rcn, DET_MERGE, (unsigned long)a2, sz2, 0);
